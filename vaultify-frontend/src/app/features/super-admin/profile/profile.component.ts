...
export class ProfileComponent implements OnInit {
  ...
  loadProfile(): void {
    ...
    this.apiService.getSuperAdminProfile().pipe(timeout(8000)).subscribe({
      next: (response) => {
  ...
  updateProfile(): void {
    ...
    this.apiService.updateSuperAdminProfile(this.profileForm).pipe(timeout(8000)).subscribe({
      next: (response) => {
        this.profile = response;
  ...
  changePassword(): void {
    ...
    this.apiService.changeSuperAdminPassword(this.passwordForm).pipe(timeout(8000)).subscribe({
      next: () => {
